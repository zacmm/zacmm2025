...
// InviteIdResponse is used for returning and auditing the invite_id and team_id.
type InviteIdResponse struct {
	InviteId string `json:"invite_id"`
	TeamId   string `json:"team_id"`
}

func (r InviteIdResponse) Auditable() map[string]any {
...
func (api *API) InitTeam() {
	...
	api.BaseRoutes.Team.Handle("/invite_id", api.APISessionRequired(getInviteId)).Methods(http.MethodGet)
...
func getInviteId(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	auditRec := c.MakeAuditRecord("getInviteId", audit.Fail)
	audit.AddEventParameter(auditRec, "team_id", c.Params.TeamId)
	defer c.LogAuditRec(auditRec)

	inviteId, err := c.App.GetInviteId(c.Params.TeamId)
	if err != nil {
		c.Err = err
		return
	}

	response := InviteIdResponse{
		InviteId: inviteId,
