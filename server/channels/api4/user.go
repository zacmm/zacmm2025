...
func (api *API) InitUser() {
	...
	// Whitelist endpoints
	api.BaseRoutes.User.Handle("/whitelist", api.APISessionRequired(getUserWhitelist)).Methods(http.MethodGet)
	api.BaseRoutes.User.Handle("/whitelist", api.APISessionRequired(addUserToWhitelist)).Methods(http.MethodPost)
	api.BaseRoutes.User.Handle("/whitelist", api.APISessionRequired(removeUserFromWhitelist)).Methods(http.MethodDelete)
}
...
// getUserWhitelist gets all whitelisted IP addresses for a user
func getUserWhitelist(c *Context, w http.ResponseWriter, r *http.Request) {
	c.RequireUserId()
	if c.Err != nil {
		return
	}

	// Check permissions - users can only view their own whitelist, admins can view any
	if !c.App.SessionHasPermissionToUser(*c.AppContext.Session(), c.Params.UserId) {
		c.SetPermissionError(model.PermissionEditOtherUsers)
		return
	}

	ips, err := c.App.GetUserWhitelistIPs(c.Params.UserId)
	if err != nil {
		c.Err = err
		return
	}
...
func addUserToWhitelist(c *Context, w http.ResponseWriter, r *http.Request) {
	c.RequireUserId()
	if c.Err != nil {
		return
	}

	// Check permissions - users can only modify their own whitelist, admins can modify any
	if !c.App.SessionHasPermissionToUser(*c.AppContext.Session(), c.Params.UserId) {
		c.SetPermissionError(model.PermissionEditOtherUsers)
		return
	}
	...
	if jsonErr := json.NewDecoder(r.Body).Decode(&requestBody); jsonErr != nil {
		c.SetInvalidParamWithErr("request body", jsonErr)
		return
	}

	if requestBody.IP == "" {
		c.SetInvalidParam("ip")
		return
	}

	auditRec := c.MakeAuditRecord("addUserToWhitelist", audit.Fail)
	defer c.LogAuditRec(auditRec)
	audit.AddEventParameter(auditRec, "user_id", c.Params.UserId)
	audit.AddEventParameter(auditRec, "ip", requestBody.IP)

	if err := c.App.AddUserToWhitelist(c.AppContext, c.Params.UserId, requestBody.IP); err != nil {
		c.Err = err
		return
	}
...
func removeUserFromWhitelist(c *Context, w http.ResponseWriter, r *http.Request) {
	c.RequireUserId()
	if c.Err != nil {
		return
	}
	...
	auditRec := c.MakeAuditRecord("removeUserFromWhitelist", audit.Fail)
	defer c.LogAuditRec(auditRec)
	audit.AddEventParameter(auditRec, "user_id", c.Params.UserId)
	audit.AddEventParameter(auditRec, "ip", requestBody.IP)

	if err := c.App.RemoveUserFromWhitelist(c.AppContext, c.Params.UserId, requestBody.IP); err != nil {
		c.Err = err
		return
	}
